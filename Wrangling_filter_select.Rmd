...
movies %>%      
  filter(year >= 1965) %>%
 filter(length > 60) %>%
  filter(votes > 500, rating <= 9) %>%
  filter(rating == "PG-13")
```

# give me only movies where the year is greater than or equal to 1965
 filter(year >= 1965) %>%
...
```
  ...
  There was 1,248 movies released in 1995. 
  
  
2. Which Comedies with an MPAA rating, over 120 minutes have a rating above 8?

```{r}
long_highrated_comedies <- movies %>%
  
filter(Comedy == 1, !mpaa == "", rating > 8, length > 120) %>%
select(title, year, rating, length, mpaa)

long_highrated_comedies
```

  There were 4 Comedies which were Big Fish, Forrest Gump, Homen Que Copiava O, and the incredible with an MPAA rating over 120 minutes having a rating above 8.
...
```
  ...
  The Harry Potter movies included in this data are Harry Potter and the Chamber of Secrets, Harry Potter and the Prisoner of Azkaban, and Harry Potter and the Sorcerer's Stone. The highest rated is Harry Potter and
with a rating or 7.8. Harry Potter and the sorcerer's stone has the most votes on the prisoner of Azkaban imdb.com 46741. 

4. What is the highest budget for a movie that is an Animation or Comedy with at least 100 votes?

```{r}
long_highrated_comedies_animation <- movies %>%
  
filter(Comedy == 1,Animation ==1,  votes > 100) %>%
select(budget, votes) %>%
  arrange(desc(budget))

long_highrated_comedies_animation
```

  The highest budget for a movie that is an animation or a comedy is Wild Wild West, their budget is 170,000,000 with 19078
